...
class ColorInput extends Widget {
  ...
  componentDidMount() {
    ...
    if (!$(target).is(`.${this.props.prefixCls}-` + this.instanceId) &&
      !$(target).closest(`.${this.props.prefixCls}-` + this.instanceId).length &&
      !$(target).is(`.${this.props.prefixCls}-colorpicker-` + this.instanceId) &&
      !$(target).closest(`.${this.props.prefixCls}-colorpicker-` + this.instanceId).length) {
  ...
  renderColorPicker(data) {
    ...
    ReactDOM.render(<div style={{
        ...
        <ColorPicker className={`${prefixCls}-component ${prefixCls}-colorpicker ${prefixCls}-colorpicker-${this.instanceId}`}
  ...
  render() {
    const props = this.props,
          prefixCls = props.prefixCls;
    return (<input ref="input" type="text" value={props.value} readOnly={true} maxLength="7" size="7"
                   className={`${prefixCls} ${prefixCls}-${this.instanceId} ${props.className || ''}`}
